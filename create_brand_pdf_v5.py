...
def get_path(name, folder):
    ...
    paths = [f"{base}/白茶品牌/{folder}/{name}.jpg", f"{base}/{folder}/{name}.jpg"]
    for p in paths:
        if os.path.exists(p):
            return p
    return None
...
def get_qr_path():
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    p = f"{base}/用户上传/photo-1777830978310.jpg"
...
def draw_product_detail(c, product):
    ...
    # 上半部分 - 山客形象（居中偏上）
    portrait_path = get_path(product["name"], "山客形象")
    if portrait_path:
        ...
        if portrait_temp:
            ...
            # 图片高度为页面上半部分
            display_height = (PAGE_HEIGHT - 80) / 2 - 20
            display_width = img_w * (display_height / img_h)
            if display_width > PAGE_WIDTH * 0.4:
                display_width = PAGE_WIDTH * 0.4
                display_height = img_h * (display_width / img_w)
            img_x = (PAGE_WIDTH - display_width) / 2
            img_y = PAGE_HEIGHT - 80 - display_height
            c.drawImage(portrait_temp, img_x, img_y, width=display_width, height=display_height, preserveAspectRatio=True)
    
    # 分隔线
    c.setStrokeColor(GOLD)
    c.setLineWidth(0.5)
    mid_y = PAGE_HEIGHT / 2
    c.line(60, mid_y, PAGE_WIDTH - 60, mid_y)
    
    # 下半部分 - 文字内容（使用全宽）
    content_top = mid_y - 30
    left_margin = 60
    max_width = PAGE_WIDTH - 120  # 左右各60pt边距
    
    # 茶品名 + 季节（两列布局）
    c.setFillColor(DARK_TEXT)
    c.setFont('MicroHei', 14)
    c.drawString(left_margin, content_top, product["teaName"])
    c.setFillColor(GOLD)
    c.setFont('MicroHei', 12)
    c.drawString(left_margin + 120, content_top, f"· {product['season']} ·")
    ...
    story = product["story"]
    # 根据可用宽度计算每行字符数（中文约11pt宽）
    chars_per_line = int(max_width / 11)
    story_lines = [story[i:i+chars_per_line] for i in range(0, len(story), chars_per_line)]
    ...
    c.drawString(left_margin, y, "风味笔记")
    y -= 18
    
    c.setFillColor(DARK_TEXT)
    c.setFont('MicroHei', 10)
    flavor_lines = [product["flavor"][i:i+chars_per_line] for i in range(0, len(product["flavor"]), chars_per_line)]
    ...
    c.setFillColor(ANNOTATION_GRAY)
    c.setFont('MicroHei', 10)
    c.drawString(left_margin, y, "收藏价值")
    y -= 18
    
    c.setFillColor(GOLD)
    c.setFont('MicroHei', 10)
    value_lines = [product["value"][i:i+chars_per_line] for i in range(0, len(product["value"]), chars_per_line)]
...
def create_brand_pdf():
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "南山帖_品牌手册.pdf")
