...
def draw_seal(c, x, y, size=30):
    ...
    c.drawString(x + size * 0.2, y + size * 0.35, '南')
...
class ProductDetailPage:
    ...
    def __init__(self, canvas, doc, product):
        ...
        img_top_y = PAGE_HEIGHT - 50
        img_bottom_y = 50
        
        if portrait_path:
            portrait_temp = compress_image_for_pdf(portrait_path, max_width=320, quality=80)
            if portrait_temp and os.path.exists(portrait_temp):
                # 左侧图片区域
                img_area_height = img_top_y - img_bottom_y
                ...
                img_x = 50
                img_y = img_bottom_y + (img_area_height - display_height) / 2
        ...
        flavor_lines = [flavor[i:i+30] for i in range(0, len(flavor), 30)]
        ...
        self.c.setFont('MicroHei', 11)
        value = product["value"]
        value_lines = [value[i:i+30] for i in range(0, len(value), 30)]
        for line in value_lines:
            self.c.drawString(right_x, y, line)
            y -= 20
        
        # 右下角朱红方印 - 放在页面最右下角，与内容分开
