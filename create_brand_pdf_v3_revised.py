...
南山帖品牌手册PDF - 修订版V3
基于v7版本修改：
1. 更新PRODUCTS文案：flavor(风味笔记)、value(藏养)、新增grade(品第)
2. 渲染"藏养"替代"收藏价值"
3. 新增"品第"渲染（朱红色）
"""
...
# ==================== 配置 ====================
OUTPUT_PATH = "./白茶品牌/南山帖_品牌手册_修订版V3.pdf"
...
def prepare_product_image(path, target_width=PRODUCT_IMG_WIDTH):
    """产品图：center crop为正方形，再缩放到统一宽度"""
    try:
        img = Image.open(path)
        ...
        # Resize to target width (正方形，所以宽高相同)
...
def add_info_to_product_image(processed_img, tea_name, season, output_path):
    """在产品图上添加茶品名、季节和方印"""
    try:
        # 使用已处理成正方形的图片
        ...
        try:
            font_small = ImageFont.truetype(FONT_TITLE, int(min(w, h) * 0.045))
        except:
        ...
        # 右下角方印
        seal_size = 45
        draw_seal(draw, w - seal_size - 15, h - seal_size - 15, seal_size)
...
def add_seal_to_ink_image(ink_path, output_path, target_width=PRODUCT_IMG_WIDTH):
    """水墨图：等比缩放至正方形框内（黑色填充），+ 叠加朱红方印，保证与产品图完全对称且内容完整"""
    try:
        ...
        # 创建正方形黑色画布，居中粘贴
        canvas = Image.new('RGB', (target_width, target_width), (10, 10, 10))
        paste_x = (target_width - new_w) // 2
        paste_y = (target_width - new_h) // 2
        ...
        # 只画右下角朱红方印，不叠加山客名
        seal_size = 50
        draw_seal(draw, target_width - seal_size - 20, target_width - seal_size - 20, seal_size)
        
        canvas.save(output_path, 'JPEG', quality=85, optimize=True)
        print(f"水墨图处理完成（正方形{target_width}x{target_width}，等比缩放+黑底）: {output_path}")
        return output_path
    except Exception as e:
        print(f"处理水墨图失败: {e}")
        return ink_path
...
def process_all_images():
    ...
    for p in PRODUCTS:
        ...
        processed_ink = add_seal_to_ink_image(p['ink_img'], ink_output)
        
        # 产品图：center crop为正方形，不叠加任何文字
        product_output = f"./白茶品牌/images/processed/{name}_product.jpg"
        
        square_img = prepare_product_image(p['product_img'], PRODUCT_IMG_WIDTH)
        if square_img:
            square_img.save(product_output, 'JPEG', quality=85, optimize=True)
            processed_product = product_output
        else:
            processed_product = p['product_img']
...
def create_product_detail_page(c, width, height, product, processed_images):
    ...
    # 上半部分：左右并列图片区（统一尺寸，保证对称）
    img_area_height = height * 0.50
    img_bottom_y = height * 0.40
    gap = 10
    img_display_w = (width - MARGIN * 2 - gap) / 2
    img_display_h = img_area_height
    ...
    try:
        img_x = MARGIN + img_display_w + gap
    ...
    for line in story_lines:
        if current_y > text_bottom_y + 105:
    ...
    c.setFillColor(ANNOTATION_GRAY)
    c.setFont('WenQuanYiZenHei', 10)
    c.drawString(MARGIN, current_y, "藏养")
    current_y -= 12
    ...
    c.setFillColor(VERMILLION)
    c.setFont('WenQuanYiZenHei', 10)
    c.drawString(MARGIN, current_y, "品第")
    current_y -= 12
    
    c.setFillColor(VERMILLION)
    c.setFont('WenQuanYiMicroHei', 9)
    grade_lines = wrap_text(product['grade'], c, 'WenQuanYiMicroHei', 9,
                           width - MARGIN * 2)
    for line in grade_lines:
        if current_y > text_bottom_y + 10:
            c.drawString(MARGIN, current_y, line)
            current_y -= 11
    ...
    c.setFillColor(WARM_GOLD)
    c.setFont('WenQuanYiMicroHei', 10)
    quote_text = product['quote']
    quote_bbox = c.stringWidth(quote_text, 'WenQuanYiMicroHei', 10)
    quote_x = (width - quote_bbox) / 2
    c.drawString(quote_x, text_bottom_y + 3, quote_text)
...
def create_brand_manual_v3():
    ...
    print("正在处理图片...")
    print("  - 产品图：center crop为正方形，统一宽度600px")
    print("  - 水墨图：只叠加朱红方印，不叠加山客名")
